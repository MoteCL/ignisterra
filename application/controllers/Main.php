...
class Main extends CI_Controller
{
    ...
    public function saveActividades()
    {
        ...
        $this->form_validation->set_rules('TotalHrs', 'TotalHrs', 'required', array(
            'TotalHrs' => 'Falta calculo'
        ));

        if ($this->form_validation->run()) {
            //$data = $this->input->post();
            $data['fecha']         = date('Y-m-d');
            $data['horaActividad'] = date('H:i:s');
            $data['orden']         = $this->input->post('orden');
            $data['TotalHrs']      = $this->input->post('TotalHrs');
            $data['id_tecnico']    = $this->input->post('id_tecnico');
            $data['actividad']     = $this->input->post('actividad');
            $data['Comentario']    = $this->input->post('Comentario');
            $data['TotalHrs']      = $this->input->post('TotalHrs');
            $data['NroSolicitud']  = $this->input->post('NroSolicitud');
