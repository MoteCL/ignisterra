...
class Seguimiento extends CI_Controller
{
  ...
  {
      $this->form_validation->set_rules('NroSolicitud', 'NroSolicitud', 'trim');
      $this->form_validation->set_rules('idMan_Tecnico', 'idMan_Tecnico', 'trim');
      if ($this->form_validation->run()) {
          $id_solicitud     = $this->input->post('NroSolicitud');
          $idMan_Tecnico    = $this->input->post('idMan_Tecnico');
          $update['estado'] = 'ABIERTA';
          $this->seguimiento->updateSeguimiento($update, $idMan_Tecnico);
          $result           = $this->ma->updateMantencion($update, $id_solicitud);
          if ($result) {
              $this->session->set_flashdata('success_msg', 'Mantencion Abierta');
          } else {
              $this->session->set_flashdata('error_msg', 'Error BD');
          }
          redirect('seguimiento/MAN_Seguimiento');
      }
  }
